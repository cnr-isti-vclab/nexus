import { getUint32 } from './Binary.js'
...
_Cache.prototype = {
    ...
    requestNodeTexture: function(mesh, id) {
	    ...
	    if(!mesh.vertex.UV_0) return;

	    let tex = mesh.patches[mesh.nfirstpatch[id]*4+2]; // Updated to 4-field patches
	    mesh.texref[tex]++;
	    mesh.status[id]++;

		let url = mesh.deepzoom ? mesh.baseurl + tex + ".jpg" : mesh.url;
		let start = mesh.deepzoom ? 0 : mesh.textures[tex*2];              // offset
		let end = mesh.deepzoom ? 0 : mesh.textures[tex*2] + mesh.textures[tex*2+1]; // offset + size
		
		// Use arraybuffer for v3 (need to parse headers), blob for v2 (direct image)
		let responseType = mesh.v3 ? 'arraybuffer' : 'blob';
    ...
    loadNodeTexture: function(request, mesh, id, texid) {
        ...
        // For v3, parse texture headers from ArrayBuffer
        if(mesh.v3) {
            view = new DataView(request.response);
            view.offset = 0;
            ntex = getUint32(view);
            mesh.status[id] += ntex - 1; // Update status for additional textures
        }
        
        // Process each texture in the group
        for(let i = 0; i < ntex; i++) {
            ...
            if(mesh.v3) {
                ...
                offset = view.offset;
                view.offset += size;
                // Create blob from portion of ArrayBuffer
                blob = new Blob([new Uint8Array(request.response, offset, size)]);
            } else {
                // For v2, response is already a blob - use it directly
            ...
            let callback = (img) => {
                if(mesh.status[id] == 0) //call was aborted.
                    return;
                mesh.createTexture(texid, img, texindex);
    ...
    removeNode: function(mesh, id) {
	    ...
	    const tex = mesh.patches[mesh.nfirstpatch[id]*4+2]; //TODO assuming one texture per node
