...
void KDTreeCloud::findRealMiddle(KDCell &node) {
    ...
    if(overlap != 0.0f) {
        node.start = node.middle - overlap*(node.middle - tmp.front());
        node.end = node.middle + overlap*(tmp.back() - node.middle);
    }
}
...
void KDTreeCloud::splitNode(KDCell &node, KDCell &child0, KDCell &child1) {
    ...
    for(quint32 i = 0; i < source.size(); i++) {
        ...
        bool remain = true;
        if(overlap == 0.0f) {
            remain = r < node.middle;
        } else {
            float t = (r - node.start)/(node.end - node.start);
            float s = (double)rand() / ((double)(RAND_MAX)+(double)(1));
            remain = (t < s);
